...
class _UploadDetailPageState extends ConsumerState<UploadDetailPage> {
  ...
  List<UploadStatus?> _assignItemsToSlots(List<UploadStatus> uploadingItems) {
    final slots = List<UploadStatus?>.filled(_maxSlots, null);
  ...
  Widget build(BuildContext context) {
    final uploadItems = ref.watch(backupProvider.select((state) => state.uploadItems));
  ...
  Widget _buildTwoSectionLayout(
    ...
    List<UploadStatus> uploadingItems,
  ...
  Widget _buildErrorCard(BuildContext context, UploadStatus item) {
    return Card(
      elevation: 0,
  ...
  Future<void> _showFileDetailDialog(BuildContext context, UploadStatus item) {
    return showDialog(
      context: context,
